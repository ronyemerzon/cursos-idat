...
- **Base de Datos:** Microsoft SQL Server (`EduPortalDB`)
- **Enlace del Repositorio en GitHub:** [https://github.com/ronyemerzon/cursos-idat](https://github.com/ronyemerzon/cursos-idat)
...
```typescript
// Implementación en role.guard.ts
export const roleGuard: CanActivateFn = (route: ActivatedRouteSnapshot) => {
  ...
  if (!authService.isAuthenticated()) {
    notification.warning('Acceso Restringido', 'Inicia sesión para continuar.');
  ...
  notification.error('Permiso Denegado', 'No cuentas con privilegios suficientes.');
...
2. Spring Boot valida la contraseña con BCrypt y genera un token JWT firmado con algoritmo **HMAC-SHA256**.
...
Todos los servicios (`UserService`, `CourseService`, `AuthService`) utilizan `HttpClient` y Observables RxJS con tipado estricto:
...
| **Usuarios** | `POST` | `/api/users` | Creación de nuevo usuario en SQL Server |
...
| **Cursos** | `POST` | `/api/courses/{id}/enroll` | Matrícula de estudiante con control de cupos |
...
3. La arquitectura desacoplada con backend Spring Boot 3 y Microsoft SQL Server garantiza un rendimiento óptimo y mantenibilidad profesional para futuras expansiones del sistema educativo.
